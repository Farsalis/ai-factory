"""Augment 'Breaking Better' validation JSONL with tool-calling examples for Mistral-7B.

This script creates validation variants by injecting tool calls and responses,
matching bb_training_data_v7.jsonl. Context-aware tool selection by user message.

Usage:
    python augment_validation_data.py --input <input.jsonl> \\
        --output augmented_validation_data.jsonl --num_variants 5
"""
...
def _select_relevant_tools(user_msg: str, tools: list[str]) -> list[str]:
    """Select relevant tools based on user message content (validation-specific logic).
    ...
    Returns:
        Filtered list of relevant tools, or original tools if no match.
    """
    user_msg_lower = user_msg.lower()

    # Context-aware tool selection for validation data
    if "presentation" in user_msg_lower:
        relevant = ["search_web", "read_file", "write_file"]
    elif "onboard" in user_msg_lower:
        relevant = ["calendar_tool", "task_tracker_tool"]
    elif "phone addiction" in user_msg_lower:
        relevant = ["task_tracker_tool", "calendar_tool"]
    elif "ai software" in user_msg_lower:
        relevant = ["search_web", "news_tool"]
    elif "novel writing" in user_msg_lower:
        relevant = ["write_file", "task_tracker_tool"]
    elif "negative self-talk" in user_msg_lower:
        relevant = ["task_tracker_tool", "write_file"]
    elif "discipline" in user_msg_lower:
        relevant = ["task_tracker_tool", "calendar_tool"]
    else:
        return tools.copy()

    # Filter to only include tools that are in the original tools list
    return [t for t in relevant if t in tools] or tools.copy()
...
def generate_tool_variant(
    ...
    Matches bb_training_data_v7.jsonl format and includes context-aware tool
    selection based on user message content (validation-specific feature).
    ...
    # Context-aware tool selection (validation-specific feature)
    relevant_tools = _select_relevant_tools(user_msg, tools)
    ...
    if variant_type == "none":
        ...
        tool_name = random.choice(relevant_tools)  # noqa: S311
        ...
        num_tools = min(2, len(relevant_tools))
        selected_tools = random.sample(relevant_tools, num_tools)
...
def augment_dataset(
    ...
    """Augment the validation dataset by creating variants per original example.

    Creates multiple variants of each original example with different tool
    injection patterns (single tool, multiple tools, or no tools), using
    ...
    """
    logger.info(
        f"Augmenting validation dataset: {len(original_data)} examples, "
...
def main() -> None:
    """CLI entry point for validation dataset augmentation."""
    parser = argparse.ArgumentParser(
        description="Augment validation JSONL dataset for tool-calling fine-tuning.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument(
        "--input", required=True, type=Path, help="Path to input validation JSONL file"
    )
    parser.add_argument(
        "--output",
        required=True,
        type=Path,
        help="Path to output augmented validation JSONL file",
    )
    parser.add_argument(
        "--num_variants",
        type=int,
    ...
    try:
        ...
        logger.info(f"Loaded {len(original_data)} original validation examples")
        ...
        logger.info("Validation augmentation pipeline completed successfully")
    except (FileNotFoundError, ValueError, json.JSONDecodeError) as e:
        logger.error(f"Validation augmentation error: {e}", exc_info=True)
        raise
    except Exception as e:
        logger.error(
            f"Unexpected error during validation augmentation: {e}", exc_info=True
        )
        raise
